...
public interface TaskRepository {
    ...
    Optional<TaskEntity> selectById(@Param("taskId") long taskId);//@Paramは引数をSELECT文でパラメータとして使えるようにしてる

    @Insert("""
            INSERT INTO tasks(summary, description, status)
            VALUES (#{task.summary}, #{task.description}, #{task.status})
            """)
    void insert(@Param("task") TaskEntity newEntity);//value内で参照できるように@Paramで("task")を指定
